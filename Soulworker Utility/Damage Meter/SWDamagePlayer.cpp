...
namespace {
	bool SkillNamesMatchForCast(const char* left, const char* right) {
...
void SWDamagePlayer::AddDamage(uint64_t totalDMG, uint64_t soulstoneDMG, SWPACKETDAMAGE_DAMAGETYPE damageType, unsigned short maxCombo, uint32_t monsterID, uint32_t skillID)
{
	...
	AddSkillCastDamage(skillID, damageType.CRIT != 0);
	Sort();
}

void SWDamagePlayer::AddSkillCastDamage(uint32_t skillID, bool isCrit)
{
	if (_skillCastStates.empty())
		return;

	char damageName[64] = { 0 };
	SWDB.GetSkillName(skillID, damageName, sizeof(damageName));
	for (auto cast = _skillCastStates.begin(); cast != _skillCastStates.end(); cast++) {
		char castName[64] = { 0 };
		SWDB.GetSkillName(cast->first, castName, sizeof(castName));
		if (cast->first != skillID && !SkillNamesMatchForCast(castName, damageName))
			continue;

		cast->second._hitCount++;
		if (isCrit)
			cast->second._critHitCount++;
	}
}
...
void SWDamagePlayer::AddSkillUsed(uint32_t skillId)
{
	...
	SkillCastState& castState = _skillCastStates[skillId];
	castState._sequence = ++_skillCastSequence;
	castState._hitCount = 0;
	castState._critHitCount = 0;
...
bool SWDamagePlayer::GetSkillCastHits(uint32_t skillID, uint32_t& hitCount, uint32_t& critHitCount) const
{
	hitCount = 0;
	critHitCount = 0;
	if (skillID == 0 || _skillCastStates.empty())
		return false;

	char damageName[64] = { 0 };
	SWDB.GetSkillName(skillID, damageName, sizeof(damageName));
	const SkillCastState* latestCast = nullptr;
	for (auto cast = _skillCastStates.begin(); cast != _skillCastStates.end(); cast++) {
		char castName[64] = { 0 };
		SWDB.GetSkillName(cast->first, castName, sizeof(castName));
		if (cast->first != skillID && !SkillNamesMatchForCast(castName, damageName))
		...
		if (latestCast == nullptr || cast->second._sequence > latestCast->_sequence)
			latestCast = &cast->second;
	}

	if (latestCast == nullptr)
		return false;

	hitCount = latestCast->_hitCount;
	critHitCount = latestCast->_critHitCount;
	return true;
}
