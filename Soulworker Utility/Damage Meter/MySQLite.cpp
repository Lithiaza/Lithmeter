...
MySQL::MySQL() : _db(nullptr), _memdb(nullptr), _skill_stmt(nullptr), _monster_stmt(nullptr), _map_stmt(nullptr), _map_stmt_eng(nullptr), _buff_stmt(nullptr) {
...
void MySQL::FreeDB() {
	...
	if (_map_stmt != nullptr) {
		sqlite3_finalize(_map_stmt);
		_map_stmt = nullptr;
	}

	if (_map_stmt_eng != nullptr) {
		sqlite3_finalize(_map_stmt_eng);
		_map_stmt_eng = nullptr;
	}

	if (_buff_stmt != nullptr) {
		sqlite3_finalize(_buff_stmt);
		_buff_stmt = nullptr;
	}
...
bool MySQL::Init() {
	...
	bool success = TRUE;
	// Recreate the prepared statements when the UI language changes (the
	// selected Name_* columns are language-dependent). This is also harmless
	// on first startup because the handles are initialized to nullptr.
	FreeDB();
