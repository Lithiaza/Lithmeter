...
private:
	struct SkillCastState {
		...
		SkillCastState() : _sequence(0), _hitCount(0), _critHitCount(0) { }
	};
	...
	uint64_t _skillCastSequence = 0;

	std::vector<SWDamageMonster*> _monsterInfo;
	std::map<uint32_t, SkillCastState> _skillCastStates;
	...
	void AddSkillCastDamage(uint32_t skillID, bool isCrit);
	void Sort();
...
public:
	...
	bool GetSkillCastHits(uint32_t skillID, uint32_t& hitCount, uint32_t& critHitCount) const;
