...
private:
	...
	float _refreshTime;
	float _meterOpacity;
	bool _meterTransparencyEnabled;
	BOOL _isTopMost;
	bool _isTableOverlayMode;
	bool _isStackedMeterMode;
	BOOL _teamTA_LF;
...
public:
	...
	bool isTableOverlayMode() const;
	bool isStackedMeterMode() const;
	...
	bool ToggleTopMost();
	bool ToggleTableOverlayMode();
	bool ToggleStackedMeterMode();
	...
	const float& GetMeterOpacity();
	void SetMeterOpacity(float opacity);
	bool isMeterTransparencyEnabled() const;
	void SetMeterTransparencyEnabled(bool enabled);
