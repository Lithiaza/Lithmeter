...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Hideable)
    {
        ...
        {
            if (MenuItem("Show all###TableShowAll", NULL))
            {
                for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
                    table->Columns[other_column_n].IsEnabledNextFrame = true;
            }
            if (MenuItem("Deselect all###TableDeselectAll", NULL))
            {
                // Keep NAME available so the table and this menu can always
                // be recovered after a bulk hide operation.
                for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
                    table->Columns[other_column_n].IsEnabledNextFrame = (other_column_n == 0);
            }
            Separator();
        }
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
